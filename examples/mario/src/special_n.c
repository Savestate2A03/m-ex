...
/// @brief Original: 0x800E0DA8
/// @param fighter 
void SpecialN(GOBJ *fighter)
{
	FighterData *fighter_data = fighter->userdata;
	SpecialNFtCmd *script_var = Fighter_GetScriptVars(fighter);

	// clear subaction flags used by this special
	script_var->interruptable = 0;
	fighter_data->flags.throw_1 = 0;

	// change to special n state and update subaction
	ActionStateChange(0, 1, 0, fighter, STATE_SPECIALN, 0, 0);
	...
	fighter_data->cb.Accessory4 = SpecialN_SpawnFireballThink;
	return;
}

/// @brief Original 0x800E1040
/// @param fighter 
void SpecialAirN(GOBJ *fighter)
{
	FighterData *fighter_data = fighter->userdata;
	SpecialNFtCmd *script_var = Fighter_GetScriptVars(fighter);
	...
	// change to special n state and update subaction
	ActionStateChange(0, 1, 0, fighter, STATE_SPECIALNAIR, 0, 0);
	Fighter_AdvanceScript(fighter);
...
void IS_MarioFireballSpawn(GOBJ *gobj)
{
	...
	item_data->self_vel.X = speed * cos(angle) * item_data->facing_direction;
...
/// 0x8029b6f8
///
void CreateFireball(GOBJ *gobj, Vec3 *position, int it_kind, float facing_direction)
{
	Vec3 ecb_center_pos;
...
void SpecialN_SpawnFireballThink(GOBJ *gobj)
{
	...
	if (fighter_data->flags.throw_1 != 0)
	{
		...
		CreateFireball(gobj, &bone_position, fireball_id, fighter_data->facing_direction);
